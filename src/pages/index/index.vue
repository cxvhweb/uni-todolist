	...
	<view class="content">
		<view class="todo-header" v-if="list.length>0">
			...
			<view class="todo-header__right">
				<view class="todo-header__right-item" :class="{'active-tab':activeIndex===0}" @click="activeIndex=0">全部</view>
				<view class="todo-header__right-item" :class="{'active-tab':activeIndex===1}" @click="activeIndex=1">代办</view>
				<view class="todo-header__right-item" :class="{'active-tab':activeIndex===2}" @click="activeIndex=2">已完成</view>
			</view>
		</view>
		<!-- 没有数据 -->
		...
		<view v-else class="todo-content">
			<view :class="['todo-list',{'todo--finish':item.status}]" v-for="(item,idx) in listData" :key="idx" @click="finish(item.id)">
...
.todo--finish.todo-list::after{background-color: #cccccc;}
